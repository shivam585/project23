...
function preload()
{
  ...
  playerAnimation=loadAnimation("rutu.png")
...
rutu.addAnimation("rutu",playerAnimation)
rutu.scale=0.5
//rutu.debug=true
...
function draw() {
  ...
  textSize(30)
  textFont("Algerian")
  text("an boy wants to go to shop to take medicines ",width/3,height/4.5)
  text("for her mother but there are many obstacles ",width/3.7,height/3.7)
  text("you have to cross the roa and dont touch the river",width/3.7,height/3.1)
  text("so you have to help him",width/3.1,height/2.7)
...
gameState="play"
  ...
  if(carGroup1.isTouching(rutu)||rutu.isTouching(pool1.spt)){
  gameState="end1"
  }
...
function log4(){
  if(frameCount%40===0){
    log5=createSprite( Math.round( random([width/1.6,width/1.7,width/2,width/2.4])),height/10,100,200)
